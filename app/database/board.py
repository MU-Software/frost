...
class Post(db_module.DefaultModelMixin, db.Model):
    ...
    user: user_module.User = db.relationship(
        "User",
        primaryjoin=user_id == user_module.User.uuid,
        backref=db.backref("posts", order_by="Post.modified_at.desc()"),
    )

    board_id = db.Column(db_module.PrimaryKeyType, db.ForeignKey("TB_BOARD.uuid"), nullable=False)
    board: "Board" = db.relationship(
        "Board", primaryjoin=board_id == Board.uuid, backref=db.backref("posts", order_by="Post.created_at.desc()")
    )
...
class Comment(db_module.DefaultModelMixin, db.Model):
    ...
    user: "user_module.User" = db.relationship(
        "User",
        primaryjoin=user_id == user_module.User.uuid,
        backref=db.backref("comments", order_by="Comment.modified_at.desc()"),
    )

    post_id = db.Column(db_module.PrimaryKeyType, db.ForeignKey("TB_POST.uuid"))
    post: "Post" = db.relationship("Post", backref=db.backref("comments", order_by="Comment.created_at.desc()"))

    parent_id = db.Column(db_module.PrimaryKeyType, db.ForeignKey("TB_COMMENT.uuid"), nullable=False)
    parent: "Comment" = db.relationship(
        "Comment",
        primaryjoin="Comment.parent_id == Comment.uuid",
        remote_side="Comment.uuid",
        backref=db.backref("children", order_by="Comment.created_at.desc()"),
    )
...
class PostTagRelation(db.Model):
    ...
    post_id = db.Column(db_module.PrimaryKeyType, db.ForeignKey("TB_POST.uuid"), nullable=False)
    post: "Post" = db.relationship("Post", primaryjoin=post_id == Post.uuid, backref=db.backref("tags"))

    tag_id = db.Column(db_module.PrimaryKeyType, db.ForeignKey("TB_TAG.uuid"), nullable=False)
...
class PostLike(db_module.DefaultModelMixin, db.Model):
    ...
    post: "Post" = db.relationship(
        "Post", primaryjoin=post_id == Post.uuid, backref=db.backref("liked_by", order_by="PostLike.created_at.desc()")
    )
    ...
    user: "user_module.User" = db.relationship(
        "User",
        primaryjoin=user_id == user_module.User.uuid,
        backref=db.backref("liked_on", order_by="PostLike.created_at.desc()"),
    )
...
class PostFavorite(db_module.DefaultModelMixin, db.Model):
    ...
    post: "Post" = db.relationship(
        ...
        backref=db.backref("favorited_by", order_by="PostFavorite.created_at.desc()"),
    )
    ...
    user: "user_module.User" = db.relationship(
        "User",
        primaryjoin=user_id == user_module.User.uuid,
        backref=db.backref("favorited_on", order_by="PostFavorite.created_at.desc()"),
    )
