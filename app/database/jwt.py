...
class RefreshToken(TokenBase, db.Model, db_module.DefaultModelMixin):
    ...
    # JWT token ID
    jti = db.Column(db_module.PrimaryKeyType, db.Sequence("SQ_RefreshToken_UUID"), primary_key=True)
    # Expiration Unix Time
    exp = db.Column(db.DateTime, nullable=False)
    # Audience, User, Token holder
    user = db.Column(db_module.PrimaryKeyType, db.ForeignKey("TB_USER.uuid"), nullable=False)
    ...
    usertable: user_module.User = db.relationship(
        ...
        backref=db.backref("refresh_tokens", order_by="RefreshToken.modified_at.desc()"),
    )
