...
def IntegrityCaser_psycopg2(err, pgcode):
    def unique_column_extractor(err):
        return re.findall(r"Key \((\S+)\)=\((\S+)\) already exists.", err.__cause__.diag.message_detail)[0][0]

    def null_column_extractor(err):
        return re.findall(
            r'null value in column "(\S+)" violates not-null constraint', err.__cause__.diag.message_primary
        )

    def fk_column_extractor(err):
        return re.findall(
            r'Key \((\S+)\)=\((\S+)\) is not present in table "(\S+)".', err.__cause__.diag.message_detail
        )[0][0]
...
def IntegrityCaser(err):
    if db.engine.driver == "psycopg2":
        ...
        print(f"Integrity Caser for {db.engine.driver} " "is not implemented yet!")
        raise err
