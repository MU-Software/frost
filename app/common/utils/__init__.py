...
def is_passwordsafe(s: str, min_char_type_num: int = 2, min_len: int = 8, max_len: int = 1024) -> str:
    # Returnable case:
    #   '': OK.
...
def BackendException(
    ...
    err.response = werkzeug.wrappers.response.Response(body_json, code, custom_headers)
...
def cookie_creator(
    ...
    if not any([expires, maxage, never_expire]):
        err_msg = "At least one of the expires, maxage, never_expire" "should be set."
...
def user_cookie(data: str, secure: bool = True) -> str:
    return cookie_creator("userJWT", data, path="/", never_expire=True, secure=secure)


def delete_cookie(
    name: str, path: str = "/", domain: str = "", secure: bool = True, samesite: str = "strict", httponly: bool = True
) -> str:
