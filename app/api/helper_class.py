...
def create_response(
    ...
    response = {"success": success, "code": code, "sub_code": sub_code, "message": message, "data": data}
            ...
            return {
                ...
                "properties": {
                    ...
                    "sub_code": {
                        "type": "string",
                        "enum": [
                            self.public_sub_code,
                        ],
                    },
                    "message": {
                        "type": "string",
                    },
                    "data": {"type": "object", "properties": recursive_dict_to_openapi_obj(self.data)},
                },
            }
    ...
    def get_model_openapi_description(cls: typing.Type[dataclasses.dataclass]) -> dict:
        ...
        for name, field in cls.__dataclass_fields__.items():
            ...
            if field.type not in openapi_type_def:
                raise TypeError(
                    "Model field type must be one of these types, not {str(type(field.type))}"
...
def dict_type_check(
    type_def: dict[str, dict[str, str]], data: dict[str, typing.Any]
) -> typing.Optional[tuple[str, str, str]]:
    ...
    def __init__(
        ...
        if AuthType.Bearer in self.auth:
            if self.auth[AuthType.Bearer]:
                self.required_fields["Authorization"] = {
                    "type": "string",
                }
                self.required_fields["X-Csrf-Token"] = {
                    "type": "string",
                }
            else:
                self.optional_fields["Authorization"] = {
                    "type": "string",
                }
                self.optional_fields["X-Csrf-Token"] = {
                    "type": "string",
                }

    def __call__(self, func: typing.Callable):  # noqa: C901
        # TODO: This method is too complex! (complexity=42)
                    ...
                    if auth == AuthType.Bearer:
                        ...
                        try:
                            ...
                            access_token = jwt_module.AccessToken.from_token(
                                access_token_bearer, flask.current_app.config.get("SECRET_KEY") + csrf_token
                            )
                        ...
                        try:
                            refresh_token = jwt_module.RefreshToken.from_token(
                                refresh_token_cookie, flask.current_app.config.get("SECRET_KEY")
                            )
        ...
        if doc_str := inspect.getdoc(func):
            ...
            for k, v in self.required_fields.items():
                ...
                field_data = {"in": "header", "name": k, "required": True}
                if "description" in v:
            ...
            try:
                ...
                if not all([z in self.req_query.keys() for z in self.required_fields]):
                    return CommonResponseCase.path_required_omitted.create_response(
                        data={
                            "lacks": [z for z in self.required_fields if z not in self.req_query],
                        }
                    )
