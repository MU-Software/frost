...
class AccountDeactivationRoute(flask.views.MethodView, api_class.MethodViewMixin):
    @api_class.RequestHeader(
        auth={
            api_class.AuthType.RefreshToken: True,
        }
    )
    ...
    def post(self, req_body: dict, refresh_token: jwt_module.RefreshToken):
        ...
        if target_user.email != req_body["email"]:
            return AccountResponseCase.user_info_mismatch.create_response(data={"fields": ["email"]})
        ...
        target_tokens = (
            db.session.query(jwt_module.RefreshToken).filter(jwt_module.RefreshToken.user == target_user.uuid).all()
        )
